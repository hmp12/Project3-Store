<div class="orders">
	...
	<div>
		<button class="btn btn-success btn-lg" value="order" id="add" style="display: none"><i class="fa fa-plus"></i> Add</button>
		...
		<tbody>
			@foreach ($orders as $order)				
				<tr>
					...
					<td>{{ $order->id }}</td>
					<td>{{ empty($order->user) ? 'deleted' : $order->user->username }}</td>
					<td>{{ empty($order->user) ? 'deleted' : $order->user->name }}</td>
					<td>{{ $order->total }}</td>
